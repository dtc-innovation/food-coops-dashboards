...
module.exports.shiftsTimes = (services) => {
  return services.reduce((times, d) => {
    const time = format(d.date_begin_tz, 'HH:mm');
...
module.exports.shiftsDays = (services) => {
  return services.reduce((times, d) => {
    const time = format(d.date_begin_tz, 'YYYY-MM-DD');
...
module.exports.shifts = (services) => {
  ...
  return groupBy(
    ...
    // by shift
    (d) => format(d.date_begin_tz, 'HH:mm'),
    // by day
